// Injected on supported platform pages (TikTok video, Instagram
// post/reel, X status, etc.). Adds a floating "Save to Ambrosia"
// button. One click → opens or focuses the Ambrosia tab with the
// current URL queued for archiving.
...
function inject() {
  ...
  btn.addEventListener('click', (e) => {
    ...
    chrome.runtime.sendMessage({ type: 'open-ambrosia-with', url });
    flashSaved(btn);
  });
...
// Some platforms (TikTok, Instagram) replace the page content during
// navigation without reloading — re-inject if the FAB went away.
